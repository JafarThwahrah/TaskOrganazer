...
var secondCondition = null
function getStatus(sel) {
	firstCondition = sel.options[sel.selectedIndex].text

	let filtered =
		firstCondition && secondCondition
			? allCards.filter(
					(card) =>
						card.priority == secondCondition && card.status == firstCondition
			  )
			: firstCondition
			? allCards.filter((card) => card.priority == firstCondition)
			: secondCondition
			? allCards.filter((card) => card.priority == secondCondition)
			: allCards

	displayCards(filtered)
}
function getPriority(sel) {
	secondCondition = sel.options[sel.selectedIndex].text
	console.log(`filteredarray before ${filteredArray}`)
	let filtered =
		firstCondition && secondCondition
			? allCards.filter(
					(card) =>
...
CriticalDropdown.addEventListener('click', (event) => {
	...
	let filteredArray = allCards.filter((card) => card.status == otherFilterValue)
